PROGRAM verlet
  ...
  tau = 0.2_wp ! /s here real values
  f_a = [0.0, 0.1, 0.0] ! /kg m s^-2, constant force with x,y,z
  ...
  DO k = 1, n
    pos_kp = pos_k + tau*v_k + (tau**2)*(f_a)/(2.0_wp*m) !** für Exponenten
    f_ap = f_a
    v_kp = v_k + tau/(2.0_wp*m)*(f_a+f_ap)

   PRINT *, "x,y,z positions of the particle at the", k, "-th iteration at tau =", tau*k, "are:"
   PRINT *, "x,y,z positions of the particle at the", k, "-th iteration at time =", tau*k, "are:"
   PRINT *, pos_k
